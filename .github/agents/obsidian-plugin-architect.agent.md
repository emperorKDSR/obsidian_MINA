---
description: "Use this agent when the user asks to implement features following Obsidian best practices or needs guidance on proper plugin API usage.\n\nTrigger phrases include:\n- 'is this following Obsidian best practices?'\n- 'how should I implement this with Obsidian APIs?'\n- 'what's the best way to handle this in Obsidian?'\n- 'review this against Obsidian conventions'\n- 'make sure this uses Obsidian patterns correctly'\n- 'is there a better Obsidian way to do this?'\n\nExamples:\n- User (lead-engineer) says 'I'm implementing a custom modal, make sure it follows Obsidian patterns' → invoke this agent to validate the implementation against Obsidian UI conventions\n- User asks 'how should I store settings for this plugin?' → invoke this agent to provide guidance on proper data persistence using Obsidian's saveData/loadData APIs\n- When implementing event handlers, user says 'verify I'm using the right hooks' → invoke this agent to ensure proper event subscription/cleanup patterns\n- User implements a workspace feature and asks 'does this handle workspace changes correctly?' → invoke this agent to verify proper layout and workspace event handling"
